...
<li class="dr-dropdown menu-item <?php echo $has_other_locales ? 'menu-item-has-children' : '' ?>" id="dr-locale-selector">
  <a class="<?php echo $has_other_locales ? 'dr-dropdown-toggle' : '' ?> nav-link dr-current-locale" data-dr-locale="<?php echo $current_locale ?>" href="#">
  ...
  <ul class="dr-dropdown-menu dr-other-locales">
  <?php foreach ( $drgc_locale_options as $locale_option ) { ?>
    <?php if ( $current_locale !== $locale_option['dr_locale'] ) { ?>
      <li class="menu-item nav-item">
        <a class="dr-dropdown-item" data-dr-locale="<?php echo $locale_option['dr_locale'] ?>" href="#">
